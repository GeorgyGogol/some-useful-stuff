...
* ${PROJECT_IS_TOP_LEVEL} - с **версии 3.21**, флаг, указывающий является ли текущий проект верхнеуровневым
* ${CMAKE_SOURCE_DIR} - путь до корня проекта, где была запущена работа CMake[^1]
...
[^1]: Рекомендую не опираться на это значание, а задавать директорию проекта через отдельную переменную ProjectName_SOURCE_DIR, т.к. иногда полезно знать именно директорию текущего проекта

# Переменные поведения

Эта группа переменных влияет на поведение CMake во время сборки. Чаще используются для установки (set)

* ${CMAKE_MODULE_PATH} - ~~путь для .cmake файлов для поиска включений~~ Я хз, не завелось
* [${CMAKE_PREFIX_PATH}](#cmake_prefix_path) - список путей для .cmake файлов для поиска включений
...
Префикс установки, который будет использоваться для всех относительных путей, используемых проектом для установки. Фактически, это путь, куда будет произведена установка приложения/библиотеки.
...
Предоставляют информацию о системе. Особо не пользовался

* ${CMAKE_SYSTEM} - Название системы, на которой выполняется CMake
